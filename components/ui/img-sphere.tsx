...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
      ...
      >
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-3 right-3 w-9 h-9 bg-black/60 rounded-full text-white flex items-center justify-center hover:bg-black/80 transition-all cursor-pointer"
            >
              <X size={18} />
            </button>
          </div>
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6">
              {selectedImage.title && (
                <h3 className="text-xl font-bold mb-2 text-[#261710]">{selectedImage.title}</h3>
              )}
              {selectedImage.description && (
                <p className="text-[#5C4F47] text-sm leading-relaxed">{selectedImage.description}</p>
              )}
            </div>
          )}
        </div>
  ...
  if (!isMounted) {
    return (
      ...
      >
        <div className="text-gray-400">Loading sphere...</div>
      </div>
    );
  }
